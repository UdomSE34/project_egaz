...
def generate_waste_pdf(month):
    """
    Generate a beautifully formatted PDF report for processed waste for a specific month.
    """
    buffer = BytesIO()
    ...
    p.drawCentredString(width / 2, height - 120, "MONTHLY WASTE REPORT")
    ...
    try:
        ...
        processed_waste = summary.total_processed_waste
    except MonthlySummary.DoesNotExist:
        processed_waste = 0

    # ========== MAIN TEXT ==========
    p.setFont("Helvetica", 12)
    p.drawString(50, height - 230, "Subject:  Waste Summary")

    p.setFont("Helvetica", 11)
    p.drawString(50, height - 260, "We hereby submit the total waste for the mentioned period:")

    # ========== TABLE SECTION ==========
    table_data = [
        ["Month", "Waste (kg)"],
        [month_str, f"{processed_waste:,.2f}"],
    ]
    ...
    p.drawString(50, height - 420, "All waste collection data has been verified and approved accordingly.")
